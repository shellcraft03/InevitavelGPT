...
const index  = pc.index(process.env.PINECONE_INDEX_ENTREVISTAS).namespace('entrevistas');
...
function chunkSegments(segments, maxChars = CHUNK_SIZE) {
  ...
  function splitBuffer() {
    // 1. Último segmento terminando em fim de frase
    ...
    // 2. Último segmento terminando em pausa
    for (let i = buffer.length - 1; i > 0; i--) {
      if (/[,;]\s*$/.test(buffer[i].text)) {
        flush(buffer.slice(0, i + 1));
        buffer = buffer.slice(i + 1);
    ...
    // 3. Fallback: corta na metade do buffer
