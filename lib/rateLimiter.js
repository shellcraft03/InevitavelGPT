...
const SESSION_QUESTION_LIMIT = parseInt(process.env.SESSION_QUESTION_LIMIT || '30', 10);
...
if (hasRedisConfig) {
  ...
  sessionLimit = new Ratelimit({
    redis,
    limiter: Ratelimit.slidingWindow(SESSION_QUESTION_LIMIT, '1 h'),
    prefix: 'rl:session',
  });
}
...
export async function checkSessionLimit(sessionId) {
  ensureRateLimiterConfigured();
  if (!sessionId) return { ok: true, remaining: SESSION_QUESTION_LIMIT, resetSeconds: 0 };
  if (!sessionLimit) return inMemoryCheck(`sess:${sessionId}`, SESSION_QUESTION_LIMIT, 3_600_000);
  const { success, remaining, reset } = await sessionLimit.limit(sessionId);
  return { ok: success, remaining, resetSeconds: Math.ceil((reset - Date.now()) / 1000) };
}

export async function logBlock(ip, type) {
  if (!redis) return;
  const ts = new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo', hour12: false });
  await redis.lpush('rl:blocked', JSON.stringify({ ip, type, ts }));
}
