...
import { hasValidHumanSession, getSessionId } from './session.js';
import { verifyTurnstile } from './turnstile.js';
import { checkMinuteLimit, checkDailyLimit, checkSessionLimit, logBlock } from './rateLimiter.js';

const ALLOWED_ORIGIN = process.env.ALLOWED_ORIGIN || 'https://www.inevitavelgpt.com';
...
export async function guardChatRequest(req, res, { turnstileAction, label, t0 }) {
  ...
  const sessionId = sessionOk ? getSessionId(req) : null;
  const [okRes, rl, daily, sessionRl] = await Promise.all([
  ...
  if (!sessionRl.ok) {
    console.warn(`[rate-limit] session sessionId=${sessionId} ip=${ip}`);
    await logBlock(ip, 'session');
